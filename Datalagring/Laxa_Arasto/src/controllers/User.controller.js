...
const getUserWithUsernameQuery = async (req, res) => {
    try {
        ...
        response.length !== 0
            ? res.status(StatusCode.OK).send(response)
            : res.status(StatusCode.NOT_FOUND).send({message: 'Could not found user with username ' + req.query.username})
    } catch (error) {
        res.status(StatusCode.INTERNAL_SERVER_ERROR).send({
            message: 'Error occured while trying to retrieve uses with username: ' + req.query.userId,
            error: error.message
        })
    }
}
